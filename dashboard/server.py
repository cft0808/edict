...
def save_tasks(tasks):
    ...
    def _refresh():
        try:
            subprocess.run(
                ["python3", str(SCRIPTS / "refresh_live_data.py")], timeout=30
            )
...
def handle_task_action(task_id, action, reason):
    ...
    if action == "stop":
        ...
        # 恢复优先级：显式历史状态 > scheduler 快照 > 默认 Doing
        sched = _ensure_scheduler(task)
        resume_state = task.get("_prev_state")
        resume_org = task.get("_prev_org")

        if not resume_state:
            snapshot = sched.get("snapshot") or {}
            snap_state = snapshot.get("state")
            # 避免恢复到 Blocked/终态，优先恢复到最近稳定业务态
            if (
                snap_state
                and snap_state not in _TERMINAL_STATES
                and snap_state != "Blocked"
            ):
                resume_state = snap_state
                snap_org = snapshot.get("org")
                if snap_org and snap_org != "阻塞":
                    resume_org = snap_org

        if (
            not resume_state
            or resume_state in _TERMINAL_STATES
            or resume_state == "Blocked"
        ):
            resume_state = "Doing"

        task["state"] = resume_state
        if resume_org and resume_org != "阻塞":
            task["org"] = resume_org
        elif task.get("org") == "阻塞":
            fallback_org = {
                "Taizi": "太子",
                "Zhongshu": "中书省",
                "Menxia": "门下省",
    ...
    if action == "resume":
        ...
        _scheduler_snapshot(task, "task-action-after-resume")
    else:
...
def add_skill_to_agent(agent_id, skill_name, description, trigger=""):
    ...
    return {
        "ok": True,
        "message": f"技能 {skill_name} 已添加到 {agent_id}",
        "path": str(skill_md),
    }
...
def add_remote_skill(agent_id, skill_name, source_url, description=""):
    ...
    try:
        if source_url.startswith("http://") or source_url.startswith("https://"):
            ...
            # 从 URL 下载，带超时保护
            req = Request(
                source_url, headers={"User-Agent": "OpenClaw-SkillManager/1.0"}
            )
            try:
            ...
            return {
                "ok": False,
                "error": "不支持的 URL 格式（仅支持 https://, file://, 或本地路径）",
            }
    except Exception as e:
...
def update_remote_skill(agent_id, skill_name):
    ...
    if not source_json.exists():
        return {
            "ok": False,
            "error": f"技能 {skill_name} 不是远程 skill（无 .source.json）",
        }
    ...
    try:
        ...
        # 重新下载
        result = add_remote_skill(
            agent_id, skill_name, source_url, source_info.get("description", "")
        )
        if result["ok"]:
...
def remove_remote_skill(agent_id, skill_name):
    ...
    if not source_json.exists():
        return {
            "ok": False,
            "error": f"技能 {skill_name} 不是远程 skill，无法通过此 API 移除",
        }
    ...
    try:
        ...
        try:
            subprocess.run(
                ["python3", str(SCRIPTS / "sync_agent_config.py")], timeout=10
            )
        except Exception:
            pass
...
def push_to_feishu():
    ...
    summary = "\n".join(cat_lines)
    date_fmt = (
        date_str[:4] + "年" + date_str[4:6] + "月" + date_str[6:] + "日"
        if len(date_str) == 8
        else date_str
    )
    payload = json.dumps(
...
def handle_create_task(
    ...
    if len(title) < _MIN_TITLE_LEN:
        return {
            "ok": False,
            "error": f"标题过短（{len(title)}<{_MIN_TITLE_LEN}字），不像是旨意",
        }
    ...
    if today_ids:
        nums = [
            int(tid.split("-")[-1]) for tid in today_ids if tid.split("-")[-1].isdigit()
        ]
    ...
    return {
        "ok": True,
        "taskId": task_id,
        "message": f"旨意 {task_id} 已下达，正在派发给太子",
    }
...
def handle_review_action(task_id, action, comment=""):
    ...
    if task.get("state") not in ("Review", "Menxia"):
        return {
            "ok": False,
            "error": f"任务 {task_id} 当前状态为 {task.get('state')}，无法御批",
        }
    ...
    _scheduler_snapshot(task, f"review-after-{action}")
    task["updatedAt"] = now_iso()
    save_tasks(tasks)
...
_DISPATCH_DEDUP_WINDOW_SEC = 90
_SESSION_LOCK_BACKOFF_SEC = 240
_DISPATCH_INFLIGHT = set()
...
def _scheduler_sync_progress_from_log(task):
    sched = _ensure_scheduler(task)
    last_log_dt = _latest_progress_dt(task)
    if not last_log_dt:
        return False
    last_sched_dt = _parse_iso(sched.get("lastProgressAt"))
    if last_sched_dt and last_log_dt <= last_sched_dt:
        return False
    sched["lastProgressAt"] = last_log_dt.isoformat().replace("+00:00", "Z")
    sched["stallSince"] = None
    sched["retryCount"] = 0
    sched["escalationLevel"] = 0
    sched["lastEscalatedAt"] = None
...
def handle_scheduler_retry(task_id, reason=""):
    ...
    _scheduler_add_flow(
        task, f"触发重试第{sched['retryCount']}次：{reason or '超时未推进'}"
    )
    task["updatedAt"] = now_iso()
    save_tasks(tasks)
...
def handle_scheduler_rollback(task_id, reason=""):
    ...
    sched["lastProgressAt"] = now_iso()
    _scheduler_add_flow(
        task, f"执行回滚：{old_state} → {snap_state}，原因：{reason or '停滞恢复'}"
    )
    task["updatedAt"] = now_iso()
    save_tasks(tasks)
...
def handle_scheduler_scan(threshold_sec=180):
    ...
    for task in tasks:
        ...
        last_dispatch_error = str(sched.get("lastDispatchError") or "").lower()
        if "session file locked" in last_dispatch_error:
            last_dispatch_at = _parse_iso(sched.get("lastDispatchAt"))
            if last_dispatch_at:
                lock_backoff_sec = max(
                    60,
                    int(
                        sched.get("sessionLockBackoffSec") or _SESSION_LOCK_BACKOFF_SEC
                    ),
                )
                since_last_dispatch = max(
                    0, int((now_dt - last_dispatch_at).total_seconds())
                )
                if since_last_dispatch < lock_backoff_sec:
                    continue
        ...
        if retry_count < max_retry:
            ...
            _scheduler_add_flow(
                task, f"停滞{stalled_sec}秒，触发自动重试第{sched['retryCount']}次"
            )
            pending_retries.append((task_id, state))
            actions.append(
                {"taskId": task_id, "action": "retry", "stalledSec": stalled_sec}
            )
            changed = True
            continue
        ...
        if level < 2:
            ...
            sched["lastEscalatedAt"] = now_iso()
            _scheduler_add_flow(
                task, f"停滞{stalled_sec}秒，升级至{target_label}协调", to=target_label
            )
            pending_escalates.append(
                (task_id, state, target, target_label, stalled_sec)
            )
            actions.append(
                {
                    "taskId": task_id,
        ...
        if sched.get("autoRollback", True):
            ...
            if snap_state and snap_state != state:
                ...
                sched["lastProgressAt"] = now_iso()
                _scheduler_add_flow(
                    task, f"连续停滞，自动回滚：{old_state} → {snap_state}"
                )
                pending_rollbacks.append((task_id, snap_state))
                actions.append(
                    {"taskId": task_id, "action": "rollback", "toState": snap_state}
                )
                changed = True
    ...
    for task_id, state in pending_rollbacks:
        ...
        if rollback_task and state not in _TERMINAL_STATES:
            dispatch_for_state(
                task_id, rollback_task, state, trigger="taizi-auto-rollback"
            )
...
def handle_repair_flow_order():
    ...
    for task in tasks:
        ...
        if (
            task.get("state") == "Zhongshu"
            and task.get("org") == "中书省"
            and len(flow_log) == 1
        ):
            task["state"] = "Taizi"
            task["org"] = "太子"
                ...
                tool_calls.append(
                    {
                        ...
                        "input_preview": json.dumps(
                            c.get("input", {}), ensure_ascii=False
                        )[:100],
                    }
                )
...
def get_task_activity(task_id):
    ...
    if flow_log:
        try:
            first_at = datetime.datetime.fromisoformat(
                flow_log[0].get("at", "").replace("Z", "+00:00")
            )
            if state in ("Done", "Cancelled") and len(flow_log) >= 2:
                last_at = datetime.datetime.fromisoformat(
                    flow_log[-1].get("at", "").replace("Z", "+00:00")
                )
            else:
...
def dispatch_for_state(task_id, task, new_state, trigger="state-transition"):
    ...
    last_dispatch_at = _parse_iso(sched.get("lastDispatchAt"))
    if (
        sched.get("lastDispatchAgent") == agent_id
        and sched.get("lastDispatchStatus") == "queued"
        and last_dispatch_at
    ):
        queued_sec = max(0, int((now_dt - last_dispatch_at).total_seconds()))
        if queued_sec < _DISPATCH_DEDUP_WINDOW_SEC:
            log.info(f"⏭️ {task_id} 跳过重复派发：{agent_id} 仍在队列中 ({queued_sec}s)")
            return

    dispatch_key = f"{task_id}:{agent_id}"
    with _DISPATCH_INFLIGHT_LOCK:
        if dispatch_key in _DISPATCH_INFLIGHT:
            log.info(f"⏭️ {task_id} 跳过并发重复派发：{agent_id}")
            return
        _DISPATCH_INFLIGHT.add(dispatch_key)
    ...
    def _do_dispatch():
        try:
            ...
            for attempt in range(1, max_retries + 1):
                ...
                result = subprocess.run(
                    cmd, capture_output=True, text=True, timeout=310
                )
                if result.returncode == 0:
                ...
                if attempt < max_retries:
                    ...
                    time.sleep(5)
            is_session_locked = "session file locked" in (err or "").lower()
            status = "session-locked" if is_session_locked else "failed"
            remark = (
                f"派发会话锁冲突：{agent_id}（{trigger}）"
                if is_session_locked
                else f"派发失败：{agent_id}（{trigger}）"
            )
            ...
            _update_task_scheduler(
                ...
                lambda t, s: (
                    ...
                    _scheduler_add_flow(
                        t, f"派发超时：{agent_id}（{trigger}）", to=t.get("org", "")
                    ),
                ),
            )
        except Exception as e:
            ...
            _update_task_scheduler(
                ...
                lambda t, s: (
                    ...
                    _scheduler_add_flow(
                        t, f"派发异常：{agent_id}（{trigger}）", to=t.get("org", "")
                    ),
                ),
            )
        finally:
            with _DISPATCH_INFLIGHT_LOCK:
                _DISPATCH_INFLIGHT.discard(dispatch_key)
...
def handle_advance_state(task_id, comment=""):
    ...
    _scheduler_snapshot(task, f"advance-after-{next_state}")
    task["updatedAt"] = now_iso()
    save_tasks(tasks)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ("", "/dashboard", "/dashboard.html"):
            ...
            self.send_json(
                read_json(
                    DATA / "morning_brief_config.json",
                    {
                        "categories": [
            ...
            if not date_clean.isdigit() or len(date_clean) != 8:
                self.send_json(
                    {"ok": False, "error": f"日期格式无效: {date}，请使用 YYYYMMDD"},
                    400,
                )
                return
            ...
            if len(parts) == 2:
                ...
                self.send_json(
                    {
                        "ok": False,
                        "error": "Usage: /api/skill-content/{agentId}/{skillName}",
                    },
                    400,
                )
    ...
    def do_POST(self):
        ...
        if length > MAX_REQUEST_BODY:
            self.send_json(
                {
                    "ok": False,
                    "error": f"Request body too large (max {MAX_REQUEST_BODY} bytes)",
                },
                413,
            )
            return
        ...
        if p == "/api/morning-config":
            ...
            if unknown:
                self.send_json(
                    {"ok": False, "error": f"未知字段: {', '.join(unknown)}"}, 400
                )
                return
            ...
            if webhook and not validate_url(
                webhook,
                allowed_schemes=("https",),
                allowed_domains=("open.feishu.cn", "open.larksuite.com"),
            ):
                self.send_json(
                    {
                        "ok": False,
                        "error": "飞书 Webhook URL 无效，仅支持 https://open.feishu.cn 或 open.larksuite.com 域名",
                    },
                    400,
                )
                return
        ...
        if p == "/api/scheduler-scan":
            ...
            try:
                ...
                self.send_json(
                    {"ok": False, "error": f"scheduler scan failed: {e}"}, 500
                )
            return
        ...
        if p == "/api/repair-flow-order":
            try:
                ...
                self.send_json(
                    {"ok": False, "error": f"repair flow order failed: {e}"}, 500
                )
            return
        ...
        if p == "/api/add-skill":
            ...
            if not agent_id or not skill_name:
                self.send_json(
                    {"ok": False, "error": "agentId and skillName required"}, 400
                )
                return
        ...
        if p == "/api/add-remote-skill":
            ...
            if not agent_id or not skill_name or not source_url:
                self.send_json(
                    {
                        "ok": False,
                        "error": "agentId, skillName, and sourceUrl required",
                    },
                    400,
                )
                return
        ...
        if p == "/api/task-action":
            ...
            if not task_id or action not in ("stop", "cancel", "resume"):
                self.send_json(
                    {
                        "ok": False,
                        "error": "taskId and action(stop/cancel/resume) required",
                    },
                    400,
                )
                return
        ...
        if p == "/api/archive-task":
            ...
            if not task_id and not archive_all:
                self.send_json(
                    {"ok": False, "error": "taskId or archiveAllDone required"}, 400
                )
                return
        ...
        if p == "/api/task-todos":
            ...
            if not isinstance(todos, list) or len(todos) > 200:
                self.send_json(
                    {"ok": False, "error": "todos must be a list (max 200 items)"}, 400
                )
                return
            ...
            for td in todos:
                if not isinstance(td, dict) or "id" not in td or "title" not in td:
                    self.send_json(
                        {"ok": False, "error": "each todo must have id and title"}, 400
                    )
                    return
        ...
        if p == "/api/review-action":
            ...
            if not task_id or action not in ("approve", "reject"):
                self.send_json(
                    {
                        "ok": False,
                        "error": "taskId and action(approve/reject) required",
                    },
                    400,
                )
                return
        ...
        if p == "/api/set-model":
            ...
            if not agent_id or not model:
                self.send_json(
                    {"ok": False, "error": "agentId and model required"}, 400
                )
                return
            ...
            def apply_async():
                try:
                    subprocess.run(
                        ["python3", str(SCRIPTS / "apply_model_changes.py")], timeout=30
                    )
                    subprocess.run(
                        ["python3", str(SCRIPTS / "sync_agent_config.py")], timeout=10
                    )
...
def main():
    ...
    parser.add_argument(
        "--cors",
        default=None,
        help="Allowed CORS origin (default: reflect request Origin header)",
    )
