...
1. 创建 Research 任务（state=Researching），主任务被阻塞
2. Memory recall（强制）
3. Brainstorming（发散，先于深调）
4. 深调（收敛）：GitHub搜索+文档搜索+竞品分析
5. 调研结果写入 memory/decisions/
6. Research Done → 主任务解锁 → Doing → ETHOS自查 → Review → Done

### 阻断规则（Research 完成前）
- ⚠️ 禁止：write/edit/exec(写)/发消息
- ✅ 允许：read/纯exec/memory recall/GitHub搜索

### 边界情况
- 皇上说「快，直接做」→ 标注「紧急跳过调研」，事后补
- 跳过了 brainstorming → 复盘时标注「跳步了」

---

## 🚀 ETHOS 工程纪律（v1.2）

### 规则一：Search Before Building
exec 写操作前必须 GitHub 搜索，记录在任务备注里。

### 规则二：Boil the Lake
差距 < 2x → 必须完整实现。

### 规则三：Review Gate
Done 前必须 QAEngineer + SecurityOfficer Review。

### 规则四：Skill Preamble
spawn 时检查：linked_research / memory recall / pending review / 上下文新鲜度。

### 错误信息规范
报错必须包含：出了什么错 + 可能原因 + 下一步行动。
...
### message 必须带 target
`channel: "feishu"` + `target: "user:ou_11c0c89cbf4db641509d9b55f731abb5"`
漏任一参数，投递失败但 session 里有，容易误以为发了。
